"""
Scenario: PV with PPA/ UH purchases electricity from HECO

SET PPA PV as variable
"""
...
def define_components(m):
    ...
    #not sure that does this code do
...
"""
    Codes that original module used
    m.Limit_PPAPurchased = Constraint(
        ...
        rule=lambda m, g, tp:
            ...
            * (m.gen_max_capacity_factor[g, tp] if m.gen_is_variable[g] else 1.0)
    )
"""
    #Using this code, I would like to set that the SWITCH starts to build PPA
    #only when PPA is lower than puchase power price

    def rule(m, g, tp):
        if m.ppa_price > m.purchase_power_price:
    ...
    # Add purchases and sales to model's load balance (This is critical part to calculate the amount of purchased power)
    ...
    m.NonEnergyCharge = Expression(
        ...
        #TODO: check whether PPAPurchased should be the part of NonEnergyCharge?
...
#    switch_data.load_aug(
#        filename=os.path.join(inputs_dir, 'marginal_price_timepoint.tab'),
        # first column must show timepoint, and other column must be named 'purchase_power_price'
#        autoselect=True,
#        param=(m.marginal_power_price,)
#    )
    switch_data.load_aug(
