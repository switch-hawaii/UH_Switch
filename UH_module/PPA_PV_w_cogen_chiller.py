...
Price Scenario:
    PV: PPA
    Cogen: owned by the UH/ not PPA/ UH needs to pay for the cost of construction and natural gas prices
"""

"""
To-do list
    1. Need to find out the way to calculate Natural Gas consumpthyuion in each timepoint
    2. Find a way to put the installation costs of cogen in the monthly bill
    3. Check the code for the constraint
        gen_is_ppa[g] instead of gen_is_variable[g] in order to
        consider only PPA related generation project (line 70s)

        gen_is_cogen[g] can I use this for cogen constraints? Do I need set up a constraint for cogen?
"""
        ...
        initialize=lambda m: (
            (g, tp)
                for g in m.COGEN_GENS
                    for tp in m.TPS_FOR_GEN[g]))
    """
    Question: whether this below lines are the same as above
    def rule(m):
        reutrn (g, tp
            for g in m.COGEN_GENS
                for tp in m.TPS_FOR_GEN[g])
