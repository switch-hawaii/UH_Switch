...
def define_components(m):
   ...
   #above 10 lines of code calculate MonthlyPeakPruchasePowr and AnnualPeakPurchasedPower
    ...
    m.PurchasePower = Var(m.TIMEPONTS, within=NonNegativeReals)
    m.PurchasePowerZone = Expression(m.LOAD_ZONES, m.TIMEPOINTS, rule=lambda m, z, tp:
        m.PurchasePower[tp])
    m.Zone_Power_Injections.append('PurchasePowerZone')
    m.MonthlyPeakPruchasePower = Var(m.PERIODS, m.MONTHS, within=NonNegativeReals)
    m.Calculate_MonthlyPeakPurchasedPower = Constraint(m.TIMEPOINT, rule=lambda m, tp:
        m.MonthlyPeakPruchasePower[m.tp_perod[tp], m.ts_month[m.tp_ts[tp]]]
        >=
        m.PurchasePower[tp])
    ...
    def calc(m, p):
        ...
        return annual_total_charge
    m.DemandCharge = Expression(m.PERIODS, rule=calc)
